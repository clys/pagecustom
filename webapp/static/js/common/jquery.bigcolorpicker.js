(function ($) {
    var bigColorPicker = new function () {
        ...
        this.init = function () {
            ...
            //颜色拾取器text输入框注册事件
            $("#bigHexColorText").on('keypress keyup', function (e) {
                if (e.type === 'keypress') {
                    if (/[^A-Fa-f0-9#]/.test(e.key)) {
                        return false;
                    }
                }
                var $t = $(this), text = $.trim($t.val()).replace(/[^A-Fa-f0-9#]/g, '');
        ...
        this.showPicker = function (callback, engine, sideLength_) {
            ...
            if (engine == "L") {
                $("#bigSections").unbind("click").removeClass("bigpicker-bgimage");
                ...
                //P模式时鼠标在颜色小格上移动时获取颜色
                $("#bigSections").unbind("click").click(function (event) {
